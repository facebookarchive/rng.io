(function( window, _, $ ) {
  ...
  var App,
      ...
      failed = false,
      // Previously there was no reason to maintain a curated list
      // of allowed paths for sub initializers or detecting the web
      // root. Now that we know some devs don't follow instructions
      // we have to take on the maintenance burden of keeping a
      // "whitelist"
      paths = [
  ...
  App = {
    ...
    root: (function() {
      // We can't use
      // location.pathname === "/"
      // as a means of detecting the web root, because some "developers"
      // might not follow the instructions.
      //
      var pathname = location.pathname,
          isRoot = !paths.some(function( path ) {
            console.log( pathname.indexOf(path) );
